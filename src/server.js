...
function restoreRooms() {
  Object.entries(roomTemplates).forEach(([code, tmpl]) => {
    ...
    room.gameType = tmpl.gameType || 'Standard';
    room.roundDurationSec = tmpl.roundDurationSec || 0;
    room.countdownSec = tmpl.countdownSec || 10;
    rooms[code] = room;
...
async function persistRoomTemplate(room) {
  roomTemplates[room.code] = {
    ...
    gameType: room.gameType || 'Standard',
    roundDurationSec: room.roundDurationSec || 0,
    countdownSec: room.countdownSec || 10,
  };
...
// rank 1 = top of chain. Players can only send orders to equal/lower rank.
const ROLE_RANKS = {
  Commander: 1,
  Support:   2,
  Medic:     2,
...
// ── GAME STATE MACHINE ───────────────────────────────────────────
// States: lobby → countdown → active → ended
// lobby     = waiting for admin to start; players can join/select role/team
// countdown = game starting; n-second countdown running
// active    = game live (existing gamePaused flag still works within this)
// ended     = round over; scoreboard shown
...
function makeRoom(code, name, password) {
  return {
    ...
    orders: [
      { id: uuidv4(), from: 'SYSTEM', text: `🎯 Swindon Airsoft — ${name||code} is active. Stand by for briefing.`, priority: 'normal', ts: Date.now() }
    ],
    objectives: [],
    zones: [],
    mapTiles: 'osm',
    ...
    createdAt: Date.now(),
    // ── Game state machine ─────────────────────────────────────
    gameState: 'lobby',     // lobby | countdown | active | ended
    gameType: 'Standard',   // free text from admin
    gamePaused: false,
    roundDurationSec: 0,    // 0 = unlimited; admin sets before starting
    roundStartedAt: null,   // Date.now() when state→active
    roundEndsAt: null,      // roundStartedAt + roundDurationSec*1000
    countdownSec: 10,       // seconds for pre-game countdown
    countdownEndsAt: null,
    _countdownTimer: null,
    _roundTimer: null,
    // ── Perks / UAV / EMP ─────────────────────────────────────
    uavActive: {},
    teamCooldowns: { red: {}, blue: {} },
    empScramble: {},
    // ── Kill/score tracking ────────────────────────────────────
    scores: { red: 0, blue: 0 },
  };
...
function startCountdown(room) {
  if (room._countdownTimer) { clearTimeout(room._countdownTimer); }
  room.gameState = 'countdown';
  room.countdownEndsAt = Date.now() + room.countdownSec * 1000;
  broadcastAll(room, { type: 'game_state', state: 'countdown', endsAt: room.countdownEndsAt, countdownSec: room.countdownSec, gameType: room.gameType });
  const msg = { id: uuidv4(), from: '⭐ ADMIN', text: `⚠ GAME STARTING — ${room.countdownSec}s countdown! Take your positions!`, priority: 'high', ts: Date.now() };
  room.orders.push(msg); broadcastAll(room, { type: 'new_order', order: msg });
  room._countdownTimer = setTimeout(() => startGame(room), room.countdownSec * 1000);
}
...
function startGame(room) {
  ...
  room.roundEndsAt = room.roundDurationSec > 0 ? room.roundStartedAt + room.roundDurationSec * 1000 : null;
  room.scores = { red: 0, blue: 0 };
  // Revive all players for game start
  Object.values(room.players).forEach(p => { p.status = 'alive'; if (p._bleedTimer) { clearTimeout(p._bleedTimer); p._bleedTimer = null; } p.bleedingOut = false; p._bleedExpired = false; });
  broadcastAll(room, { type: 'game_state', state: 'active', roundStartedAt: room.roundStartedAt, roundEndsAt: room.roundEndsAt, gameType: room.gameType });
  broadcastAll(room, { type: 'game_reset' });
  const msg = { id: uuidv4(), from: '⭐ COMMAND', text: `🔴 GAME ON — ${room.gameType.toUpperCase()} — All players move out!`, priority: 'high', ts: Date.now() };
  room.orders.push(msg); broadcastAll(room, { type: 'new_order', order: msg });
  if (room.roundEndsAt) {
    room._roundTimer = setTimeout(() => endGame(room, 'Time expired'), room.roundDurationSec * 1000);
  }
  logEvent('game_started', { code: room.code, gameType: room.gameType });
}

function endGame(room, reason = 'Admin ended game') {
  if (room._roundTimer) { clearTimeout(room._roundTimer); room._roundTimer = null; }
  if (room._countdownTimer) { clearTimeout(room._countdownTimer); room._countdownTimer = null; }
  room.gameState = 'ended';
  broadcastAll(room, { type: 'game_state', state: 'ended', scores: room.scores, reason });
  const msg = { id: uuidv4(), from: '⭐ COMMAND', text: `🏁 GAME OVER — ${reason}. RED: ${room.scores.red} | BLUE: ${room.scores.blue}`, priority: 'high', ts: Date.now() };
  room.orders.push(msg); broadcastAll(room, { type: 'new_order', order: msg });
  logEvent('game_ended', { code: room.code, scores: room.scores, reason });
}
...
function resetToLobby(room) {
  ...
  room.scores = { red: 0, blue: 0 };
  Object.values(room.players).forEach(p => { p.status = 'alive'; if (p._bleedTimer) { clearTimeout(p._bleedTimer); p._bleedTimer = null; } p.bleedingOut = false; p._bleedExpired = false; });
  broadcastAll(room, { type: 'game_state', state: 'lobby', gameType: room.gameType });
  broadcastAll(room, { type: 'game_reset' });
  const msg = { id: uuidv4(), from: 'SYSTEM', text: `🔄 Returned to lobby. Awaiting next game.`, priority: 'normal', ts: Date.now() };
  room.orders.push(msg); broadcastAll(room, { type: 'new_order', order: msg });
}
...
function playerPublic(p) {
  return { id: p.id, callsign: p.callsign, team: p.team, role: p.role, rank: ROLE_RANKS[p.role] || 3,
  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type) {
      ...
      case 'order': {
        ...
        const room = rooms[roomCode]; if (!room) break;
        const player = room.players[playerId]; if (!player) break;
        const senderRank = ROLE_RANKS[player.role] || 3;
        // Chain of command: target can be 'all', 'team', or a specific playerId
        // Only commanders (rank 1) can send to all; others send to team
        const targetScope = msg.target || 'team';
        const order = {
          id: uuidv4(),
          ...
          priority: msg.priority || (senderRank === 1 ? 'high' : 'normal'),
          ts: Date.now(),
        };
        room.orders.push(order); if (room.orders.length > 200) room.orders.shift();
        if (targetScope === 'all' && senderRank === 1) {
          broadcastAll(room, { type: 'new_order', order });
        } else {
          // Team-scoped: goes to own team + admins
          broadcastTeam(room, player.team, { type: 'new_order', order });
          // Also send to admin observers
          if (room.adminObservers) room.adminObservers.forEach(ows => { if (ows.readyState === 1) ows.send(JSON.stringify({ type: 'new_order', order })); });
        }
        break;
      }

      // ── Player marking enemies (kill report) ─────────────────────
      case 'kill_report': {
        if (!playerId || !roomCode) break;
        const room = rooms[roomCode]; if (!room) break;
        const player = room.players[playerId]; if (!player) break;
        if (room.gameState !== 'active') break;
        room.scores[player.team] = (room.scores[player.team] || 0) + 1;
        broadcastAll(room, { type: 'score_update', scores: room.scores });
        break;
      }
...
// REST — Public
// ════════════════════════════════════════════════════════════════════
app.get('/health', (_, res) => res.json({ status: 'ok', rooms: Object.keys(rooms).length, uptime: Math.floor(process.uptime()), storage: db ? 'postgresql' : 'file', dbConnected: !!db }));
...
app.get('/api/admin/rooms', adminAuth, (req, res) => {
  ...
  res.json(list.map(r => ({
    ...
    gameType: r.gameType || 'Standard',
    scores: r.scores || { red: 0, blue: 0 },
    roundDurationSec: r.roundDurationSec || 0,
    countdownSec: r.countdownSec || 10,
  })));
});
...
// Game controls
// ── Game State Machine REST endpoints ───────────────────────────

// Set game type + options before starting
app.post('/api/admin/rooms/:code/game-setup', adminAuth, (req, res) => {
  const room = rooms[req.params.code.toUpperCase()]; if (!room) return res.status(404).json({ error: 'Not found' });
  if (!canAccessRoom(req.adminSession, room.code)) return res.status(403).json({ error: 'No access' });
  if (req.body.gameType !== undefined) room.gameType = String(req.body.gameType).slice(0, 60) || 'Standard';
  if (req.body.roundDurationSec !== undefined) room.roundDurationSec = Math.max(0, parseInt(req.body.roundDurationSec) || 0);
  if (req.body.countdownSec !== undefined) room.countdownSec = Math.min(60, Math.max(3, parseInt(req.body.countdownSec) || 10));
  persistRoomTemplate(room).catch(() => {});
  broadcastAll(room, { type: 'game_setup', gameType: room.gameType, roundDurationSec: room.roundDurationSec, countdownSec: room.countdownSec });
  res.json({ ok: true, gameType: room.gameType, roundDurationSec: room.roundDurationSec });
});

// Start countdown → game
app.post('/api/admin/rooms/:code/start', adminAuth, (req, res) => {
  const room = rooms[req.params.code.toUpperCase()]; if (!room) return res.status(404).json({ error: 'Not found' });
  if (!canAccessRoom(req.adminSession, room.code)) return res.status(403).json({ error: 'No access' });
  if (room.gameState === 'active') return res.status(400).json({ error: 'Game already active' });
  startCountdown(room);
  res.json({ ok: true, countdownSec: room.countdownSec });
});

// Force-skip countdown and go straight to active
app.post('/api/admin/rooms/:code/start-now', adminAuth, (req, res) => {
  const room = rooms[req.params.code.toUpperCase()]; if (!room) return res.status(404).json({ error: 'Not found' });
  if (!canAccessRoom(req.adminSession, room.code)) return res.status(403).json({ error: 'No access' });
  if (room._countdownTimer) { clearTimeout(room._countdownTimer); room._countdownTimer = null; }
  startGame(room);
...
// End game
app.post('/api/admin/rooms/:code/end-game', adminAuth, (req, res) => {
  const room = rooms[req.params.code.toUpperCase()]; if (!room) return res.status(404).json({ error: 'Not found' });
  if (!canAccessRoom(req.adminSession, room.code)) return res.status(403).json({ error: 'No access' });
  endGame(room, req.body.reason || 'Admin ended game');
  res.json({ ok: true });
});

// Back to lobby
app.post('/api/admin/rooms/:code/lobby', adminAuth, (req, res) => {
  const room = rooms[req.params.code.toUpperCase()]; if (!room) return res.status(404).json({ error: 'Not found' });
  if (!canAccessRoom(req.adminSession, room.code)) return res.status(403).json({ error: 'No access' });
  resetToLobby(room);
  res.json({ ok: true });
